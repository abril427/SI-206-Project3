...
cur.execute(query)
for row in cur:
  descriptions_fav_users.append(str(row))
...
query = "SELECT text, Users.screen_name FROM Tweets INNER JOIN Users ON Tweets.user_id = Users.user_id WHERE Tweets.retweets > 50"
cur.execute(query)
...
description_words = {word for word in descriptions_fav_users}
...
# most_common_char = Counter()
# most_common_char = Counter(descriptions_fav_users).most_common(4)
# print("This is the most common char:")
# print(most_common_char)
# most_common_char = ()
cnt = Counter()
for description in descriptions_fav_users:
	most_common_char = Counter(description).most_common(1)
most_common_char = str(most_common_char[0][0])
# print(most_common_char)
...
query = "SELECT text FROM Tweets INNER JOIN Users ON Tweets.user_id = Users.user_id"
cur.execute(query)
user_tweets = []
for row in cur:
	user_tweets.append(str(row))

query = "SELECT text, Users.screen_name FROM Tweets INNER JOIN Users ON Tweets.user_id = Users.user_id"
cur.execute(query)
for row in cur:
	# user_tweets = [tweet for tweet in row[0]]
	if row[1] not in twitter_info_diction:
		twitter_info_diction[row[1]] = user_tweets
